...
// Función para eliminar espacios en blanco al principio y al final de una
// cadena
...
void cargar_preguntas(Map *mapa_preguntas, Map *mapa_lista, Map *mapa_hard,
                      Map *mapa_easy, Map *mapa_medium) {
...
List *seleccionar_preguntas_azar(Map *mapa_preguntas, int num_preguntas) {
  ...
  for (int i = 0; i < num_preguntas; i++) {
    ...
    for (int j = 0; j < mapa_preguntas->capac; j++) {
      if (mapa_preguntas->hashArray[j] != NULL &&
          mapa_preguntas->hashArray[j]->key != -1) {
        if (index == 0) {
...
int cantidad_preguntas_configuracion =
    5; // Variable global para almacenar la cantidad de preguntas
...
int configurar_cantidad_preguntas() {
  ...
  case 4:
    ...
    break;
  default:
    printf("Opción no válida. Se mantiene la configuración actual (%d "
           "preguntas).\n",
...
List *seleccinar_dificultad(Map *mapa_hard, Map *mapa_medium, Map *mapa_easy,
                            Map *mapa_lista, Map *mapa_preguntas,
                            int num_preguntas) {
  printf("\n################################################\n");
  printf("#                                              #\n");
...
void responder_preguntas(List *lista_preguntas) {
  ...
  while (pregunta != NULL) {
    ...
    case 2:
      ...
      break;
    case 3:
      if (strcmp(((Pregunta *)lista_preguntas->current->data)->tipo,
                 "tipo lista") == 0) {
...
void responder_preguntas_versus(List *lista_preguntas, char *nombre_jugador1,
  ...
  while (pregunta != NULL) {
    ...
    case 3:
      if (strcmp(pregunta->tipo, "tipo lista") == 0) {
        ...
        char *token = strtok(respuesta, ";");
        while (token != NULL) {
          respuestas = (char **)realloc(respuestas,
                                        (num_respuestas + 1) * sizeof(char *));
          respuestas[num_respuestas] = strdup(token);
          num_respuestas++;
          token = strtok(NULL, ";");
        }
...
int main(void) {
  ...
  cargar_preguntas(mapa_preguntas, mapa_lista, mapa_hard, mapa_easy,
                   mapa_medium);
  ...
  do {
    ...
    case 1:
      ...
      List *lista_preguntas =
          seleccinar_dificultad(mapa_hard, mapa_medium, mapa_easy, mapa_lista,
                                mapa_preguntas, num_preguntas);
    ...
    case 2:
      ...
      List *lista_preguntas_versus =
          seleccinar_dificultad(mapa_hard, mapa_medium, mapa_easy, mapa_lista,
                                mapa_preguntas, num_preguntas);
      responder_preguntas_versus(lista_preguntas_versus, nombre_jugador1,
                                 nombre_jugador2);
      break;
    case 3:
